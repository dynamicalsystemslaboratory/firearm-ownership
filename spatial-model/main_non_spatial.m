...
phi1_hrr = results.beta(2); % coefficient for x1
phi1_lrr = results.beta(3); % coefficient for x1
phi2_hrr = results.beta(4); % coefficient for x2
phi2_lrr = results.beta(5); % coefficient for x2
alpha_hrr = results.beta(6);
alpha_lrr = results.beta(7);
gamma = results.beta(8);
sig = results.sige;

t_tau = results.tstat(1);
t_phi1_hrr = results.tstat(2);
t_phi1_lrr = results.tstat(3);
t_phi2_hrr = results.tstat(4);
t_phi2_lrr = results.tstat(5);
t_alpha_hrr = results.tstat(6);
t_alpha_lrr = results.tstat(7);
t_gamma = results.tstat(8);
...
for obs = 3:size(BCC_state_monthly,1)
    ...
    if m == 10 % we have a measurement of the ownership from surveys every October
        if y == 2000 % if we do not have a measurement of ownership because this is the first year of survey data
            ...
            firearm_ownership_monthly(end+1,:) = array2table([y m transpose(tau*y_t_12 + ...
